...
adds a Kin-first discovery reminder to your agent instruction files. New global entries inherit
the active workspace, while an existing user-authored `cwd` is preserved. After graph
initialization succeeds, `kin init` completes Antigravity's repo-scoped workspace entry when
setup ran first. `kin setup status` then verifies each client config.
