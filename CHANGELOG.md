...
- Admit a Git history one commit at a time instead of holding every commit's resolved tree. `kin init` now resolves each commit's tree from its first parent's and the paths that differ, keeps a hash and a content observation in the tree's place, and drops the tree once nothing later resolves against it, so a conversion's memory no longer follows commits multiplied by files. The phase-1 forecast and its refusal follow the same model: a history that was refused with "at least 582.5 GB" for 21,679 commits over 7,213 files is forecast from its commit count, its tree width and the bytes of history reachable from HEAD instead, each a floor read off a measured conversion, and the refusal names which of the three decided it. That history now admits: a normal clone of facebook/react converts in 1 hour 47 minutes at a 36.9 GB peak, against the 36.7 GB it was forecast. Because every term is now a floor rather than an over-estimate, the refusal fires at the machine's ceiling instead of half again past it, so a forecast above what the machine has stops the run in words rather than letting it die partway. Stores are unchanged on disk and a store admitted by an earlier release opens as before.
...
- Refuse a deletion verdict read off a reference class this host could produce and did not (#990)
...
- Land the advisory bump on a schedule so a merge group never meets a fresh advisory (#910)
- Make the new CI jobs release-required so the mint still verifies them (#915)
- Add kin agent, a Kin-native provider-neutral agent loop (#917)
...
- Name every dark span on the CLI commit path (#851)
...
- Auto-update by default, an unattended executor, and a no-spawn MCP probe (#849)
...
- Create missing artifact enrichment records at embed time and ship kin-db 0.7.23 (#843)
...
- Rewrite the em dashes and banned words out of the public docs (#771)
...
- Read the sentinel token from its account-suffixed secret name (#796)
...
- Register the embedding environment kin-db actually reads (#731)
...
- Match each parsed declaration to a distinct entity (#715)
...
- Retract already-admitted paths when ignore rules begin covering them (#713)
- Report every Git admission blocker before deriving any history (#714)
...
- Replace the visual identity with the current brand system (#630)
