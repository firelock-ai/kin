...
/// `kin commit` answers the same question `kin init` does, and for the same
/// reason.
///
...
/// `kin commit ... 2>&1 | head -3`, `head` took its three lines and left, and
/// the summary write that followed panicked. 101 is the panic exit status, so
/// the run that worked reported a crash inside the product.
///
/// The change is in repository authority before the summary is written, so the
/// status has to report the change and not the pipe. The control below is the
/// same commit with nobody truncating it: without it, a build that stopped
/// committing altogether would satisfy the arm.
#[test]
fn commit_reports_its_own_status_when_the_reader_closes_the_pipe() {
    let root = tempdir().expect("temp root");
    let repo = root.path().join("head-capped-commit");
    seed_linkable_git_repo(&repo);
    let runtime = IsolatedDaemonRuntime::new(&repo);
    ...
    assert!(
        init.status.success(),
        "the fixture must admit before a commit can be graded: {}",
        String::from_utf8_lossy(&init.stderr)
    );

    // The control: an untruncated commit succeeds and writes a summary.
    fs::write(repo.join("control.py"), "def control():\n    return 1\n")
        .expect("write the control edit");
    let control = runtime
        .process_command_for_test(env!("CARGO_BIN_EXE_kin"))
        .arg("commit")
        .arg("-m")
        .arg("Add a control module")
        .current_dir(&repo)
        .output()
        .expect("run kin commit");
    assert!(
        control.status.success(),
        "the control commit must succeed: stdout={} stderr={}",
        String::from_utf8_lossy(&control.stdout),
        String::from_utf8_lossy(&control.stderr)
    );
    assert!(
        !control.stdout.is_empty(),
        "the control commit must write a summary, or the truncated arm below is vacuous"
    );
    ...
    let output = runtime
        ...
        .current_dir(&repo)
        .output()
        .expect("run kin commit into a reader that closes early");
    assert!(
        output.status.success(),
        "the shell driving the pipeline must itself succeed: stderr={}",
        String::from_utf8_lossy(&output.stderr)
    ...
    assert!(
        ...
        "a reader leaving is not this program crashing: {}",
        String::from_utf8_lossy(&output.stderr)
    );
}
