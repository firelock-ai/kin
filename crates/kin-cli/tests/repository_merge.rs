...
fn conflicting_merge_is_parked_as_a_durable_transaction_and_names_what_conflicted() {
    ...
    let stderr = String::from_utf8_lossy(&merged.stdout);
    assert!(
        stderr.contains("unresolved conflict"),
        "the listing names the conflict set: {stderr}"
    );
    ...
    assert!(
        ...
        "the listing names the conflicting artifact: {stderr}"
    );
    assert!(
        stderr.contains("entity "),
        "the listing names the conflicting entity: {stderr}"
    );
...
fn merge_conflict_commands_report_no_merge_on_a_clean_workspace() {
    ...
    let listed = run_kin(&repo, &home, &["conflicts"]);
    assert!(
    ...
    let resolved = run_kin(&repo, &home, &["resolve", "--all-ours"]);
    assert!(
