...
pub async fn run_wizard(opts: WizardOptions) -> Result<()> {
    ...
    let mode = if let Some(ref m) = opts.mode {
        ...
        let modes = vec![
            "Compatibility  —  files on disk, Kin indexes alongside (recommended for existing projects)",
            "Native         —  graph is source of truth, files are projections (recommended for new projects)",
        ];
