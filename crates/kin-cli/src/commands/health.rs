...
pub async fn run_health_checks() -> HealthReport {
    ...
    checks.push(check_relation_census().await);
...
async fn check_relation_census() -> HealthCheck {
    ...
    let Some(daemon_url) = crate::daemon_client::resolve_daemon_url_if_running_async(&layout).await
        ...
        return HealthCheck::new(
            ...
            LABEL,
            HealthStatus::Unsupported,
            "n/a — no daemon running for this repository, so the relation census cannot be read;              a daemon starts on first use",
        )
        .with_manual_fix("run any `kin` command in the repo to auto-start the daemon");
    };
    let client = match crate::daemon_client::DaemonClient::from_base_url_for_layout(
        ...
        .await
    {
        ...
        Err(error) => {
            return HealthCheck::new(
                ...
                HealthStatus::Stale,
                format!(
                    "daemon reachable ({daemon_url}), but the relation census is unavailable:                      {error}"
                ),
            )
            .with_manual_fix("run `kin graph status` and resolve the reported daemon error")
        }
    };
    let Some(comparison) = response.relation_census else {
        return HealthCheck::new(
            ID,
            LABEL,
            HealthStatus::Stale,
            "the daemon serving this repository does not report a relation census; it predates              the measurement",
        )
...
pub(crate) fn relation_census_health(
    ...
    if losses.is_empty() {
        return HealthCheck::new(
            ...
            HealthStatus::Healthy,
            format!(
                "no relation kind has lost ground since the last recorded census ({} kind(s)                  compared)",
                comparison.changes.len()
            ),
        );
    }
    HealthCheck::new(ID, LABEL, HealthStatus::Stale, losses.join("; ")).with_manual_fix(
        "compare `kin graph status` against the named cause, then re-run enrichment with it          cleared",
    )
}
