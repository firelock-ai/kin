...
pub async fn run(source: Option<String>, depth: String) -> Result<()> {
    ...
    let strategy = match depth.to_lowercase().as_str() {
        ...
        _ => anyhow::bail!(
            "invalid depth '{}': expected 'shallow' or 'deep'",
            depth
        ),
    };
    ...
    let scan = kin_migrate::scan_repo(&source_path)
        .map_err(|e| anyhow::anyhow!("scan failed: {}", e))?;

    println!("  Default branch: {}", scan.default_branch.as_deref().unwrap_or("(detached)"));
    ...
    let graph = kin_graph::KuzuGraphStore::in_memory()
