...
pub async fn start(global: bool) -> Result<()> {
    let _ = global; // --global is always-on, flag kept for backwards compat
    ...
    if let Ok(registry) = kin_core::registry::KinRegistry::load() {
        for repo in &registry.repos {
            ...
            // Skip the primary (CWD) repo — also check if CWD is inside this repo
...
/// Returns `None` if no `.kin/` exists (e.g., running from a parent directory).
/// Auto-initializes if no `.kin/` is found but siblings exist (so MCP works anywhere).
...
fn merge_graphs(
    ...
    for (repo_name, sibling) in siblings {
        // Merge entities with repo-name tagging
        ...
        for entity in &entities {
            ...
            // Tag file_origin with repo name for cross-repo identification
