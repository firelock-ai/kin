...
/// parent never reads a half-written name.
///
/// The park is bounded and watches for a parent that died without signalling.
/// A process whose only exit is a signal outlives a parent killed without
/// unwinding, and it turns a swallowed signal into a run that hangs rather than
/// one that fails. Reaching either bound exits without dying of a signal, which
/// is what the parent asserts against, so the escape is loud.
#[cfg(all(test, unix))]
fn park_at_capture_barrier(capture: &Path) {
    ...
    while std::time::Instant::now() < deadline && unsafe { libc::getppid() } != 1 {
        std::thread::sleep(std::time::Duration::from_millis(50));
    }
    // Distinct from any status a real init produces, so a parent that reads it
    // is reading "nothing ever signalled me" rather than an init outcome.
    std::process::exit(87);
}
...
mod tests {
    ...
    /// An init killed while it holds a capture leaves neither `.kin` nor that
    /// capture directory.
    ///
    ...
    /// directory it staged, and parks there, so the signal always lands on a
    ...
    /// this test asserts. The name says "holding a capture" rather than
    /// "mid-capture" for that reason; the bounded-retry path that a live writer
    /// exercises is deliberately not what this asserts.
    #[cfg(unix)]
    #[test]
    fn an_init_killed_holding_a_capture_leaves_no_kin_and_no_capture_directory() {
        let root = tempfile::tempdir().unwrap();
        let source = root.path().join("source");
        std::fs::create_dir(&source).unwrap();
        initialize_git(&source);
    ...
    /// Child half of
    /// [`an_init_killed_holding_a_capture_leaves_no_kin_and_no_capture_directory`].
    ///
