...
pub fn generate_managed_content(
    target: &ManagedDocTarget,
    summary: &RepoSummary,
) -> String {
    let mut out = String::new();
            ...
            "summary" => {
                ...
                if !summary.language_breakdown.is_empty() {
                    ...
                    let parts: Vec<String> =
                        langs.iter().map(|(k, v)| format!("{} ({})", k, v)).collect();
...
fn render_kin_first_section(target_path: &str) -> String {
    ...
    out.push_str("| Instead of | Use | Why |\n");
    out.push_str("|------------|-----|-----|\n");
    out.push_str("| `git diff` | `kin review` | Entity-level diff + downstream impact + risk assessment |\n");
    ...
    out.push_str("kin search <p> --kind <k>     # Filter by kind: function, class, struct, trait\n");
    out.push_str("kin search <p> --language <l>  # Filter by language: rust, typescript, python, go, java\n");
    out.push_str("kin context <entity>          # Token-budgeted context pack\n");
    out.push_str("kin support                   # Coverage report (entities, languages, relations)\n");
...
mod tests {
    ...
    fn config_default_has_agents_md() {
        ...
        assert!(config.targets.iter().any(|t| t.path == "AGENTS.md" && t.enabled));
    }
