...
pub const DOWNSTREAM: &[EnvVarSpec] = &[
    ...
    EnvVarSpec { name: "KIN_XREF_ALLOW_FORCED_CONTENTION", kind: Kind::Bool, default: "false", sensitivity: Sensitivity::Correctness, summary: "acceptance-only gate: allows a request to ask a stable reference read (command xref, find_references, bulk_check_references) to observe a moving graph authority, so the exhausted-read refusal can be exercised deterministically instead of raced. Captured at daemon start like every other behavior lever, so a production daemon is deaf to the request field however it is set. Off by default and never set by the product; the literal 1 turns it on" },
