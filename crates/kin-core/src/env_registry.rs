...
pub const OPERATIONAL: &[EnvVarSpec] = &[
    ...
    EnvVarSpec { name: "KIN_DAEMON_NO_COVERAGE_TREE", kind: Kind::Bool, default: "false", sensitivity: Sensitivity::Operational, summary: "stop the daemon retaining the durable workspace tree at startup, so a coverage read opens this repository's authority for itself as it did before. The retained tree is what keeps `kin graph status` from paying a second whole-store open, which on a converted repository decodes a change map the read never touches; set this truthy only to reproduce that cost deliberately, since the open it restores is the whole store" },
