...
pub const DOWNSTREAM: &[EnvVarSpec] = &[
    ...
    EnvVarSpec { name: "KIN_XREF_FORCE_CONTENTION", kind: Kind::Bool, default: "false", sensitivity: Sensitivity::Correctness, summary: "acceptance-only: make every stable reference read (command xref, find_references, bulk_check_references) observe a moving graph authority, so the exhausted-read refusal can be exercised deterministically instead of raced. Off by default and never set by the product; the literal 1 turns it on" },
