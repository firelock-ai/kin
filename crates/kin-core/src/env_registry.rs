...
pub const OPERATIONAL: &[EnvVarSpec] = &[
    ...
    EnvVarSpec { name: "KIN_DAEMON_AUTH_ROTATION_MAX_ACCEPTS", kind: Kind::Usize, default: "10000", sensitivity: Sensitivity::Operational, summary: "how many requests a daemon rotation overlap window accepts on the superseded token before closing, counted durably across restarts. Subject to the same refusal of zero and of unparseable values as KIN_DAEMON_AUTH_ROTATION_WINDOW_SECS; whichever bound is reached first closes the window" },
    EnvVarSpec { name: "KIN_SUPERVISOR_AUTH_ROTATION_WINDOW_SECS", kind: Kind::Usize, default: "86400", sensitivity: Sensitivity::Operational, summary: "the supervisor counterpart of KIN_DAEMON_AUTH_ROTATION_WINDOW_SECS, bounding its own rotation overlap window with its own durable record" },
    EnvVarSpec { name: "KIN_SUPERVISOR_AUTH_ROTATION_MAX_ACCEPTS", kind: Kind::Usize, default: "10000", sensitivity: Sensitivity::Operational, summary: "the supervisor counterpart of KIN_DAEMON_AUTH_ROTATION_MAX_ACCEPTS, bounding its own rotation overlap window with its own durable record" },
    ...
    // ---- host memory pressure (what heavy work consults before it starts) ----
    EnvVarSpec { name: "KIN_MEMORY_PRESSURE", kind: Kind::OneOf(&["unknown", "nominal", "elevated", "critical"]), default: "", sensitivity: Sensitivity::Operational, summary: "force the memory-pressure level heavy work is judged against, in place of measuring the machine: 'critical' refuses the enrichment sweep, the embedding batch and ambient admission with a named reason, 'elevated' shrinks the embedding batch, 'unknown' proceeds exactly as an unreadable host does, and unset measures the cgroup or the host. Read by the daemon at process start, so it takes effect on the command that starts one" },
    EnvVarSpec { name: "KIN_MEMORY_PRESSURE_ELEVATED_FRACTION", kind: Kind::NonNegF32, default: "0.75", sensitivity: Sensitivity::Operational, summary: "fraction of the memory ceiling at or above which heavy work shrinks; must be within 0..=1, and anything else keeps 0.75" },
    ...
    EnvVarSpec { name: "KIN_SPINE_MAX_EAGER_SIBLINGS", kind: Kind::Usize, default: "16", sensitivity: Sensitivity::Operational, summary: "how many registered sibling repositories the daemon loads eagerly into the cross-repo spine at startup, each of which opens that sibling's whole workspace graph serially. Unset keeps 16; zero disables eager sibling loading entirely and the spine answers over the primary alone. A capture stopped here is BOUNDED, not incomplete: the spine says so with captured-of-registered counts and edges between captured repos stay valid, which is what separates a deliberate cap from a sibling that failed to load" },
    EnvVarSpec { name: "KIN_TRANSFER_MAX_DECODED_BODY_BYTES", kind: Kind::Usize, default: "67108864", sensitivity: Sensitivity::Operational, summary: "kin-daemon receive ceiling for one transfer pack's decoded body closure, in bytes; unset, zero or unparseable keeps the compiled 64 MiB, and the effective bound is always the smaller of this and what the sending peer advertised, so raising it never overrides a peer. It is a transport bound, not the product's answer to repository size, and it is held in memory per concurrent receive" },
    ...
    EnvVarSpec { name: "KIN_DAEMON_UNRESPONSIVE_TIMEOUT_SECS", kind: Kind::Secs, default: "20", sensitivity: Sensitivity::Operational, summary: "how long a daemon that never reported itself warming may answer nothing at all before the client calls it wedged and reports, instead of waiting out KIN_DAEMON_READY_TIMEOUT_SECS. A warming daemon answers /readiness throughout its load and never reaches this. Zero waits the full readiness budget" },
    ...
    EnvVarSpec { name: "KIN_REMOTE_HTTP_TIMEOUT_SECS", kind: Kind::Secs, default: "600", sensitivity: Sensitivity::Operational, summary: "HTTP timeout for native Kin remote repository transfer endpoints, in whole seconds; defaults to 600" },
    ...
    EnvVarSpec { name: "KIN_REMOTE_BEARER_TOKEN", kind: Kind::Secret, default: "", sensitivity: Sensitivity::Secret, summary: "bearer token for a native Kin remote when no stored auth session is present: a KinLab token, or for a peer daemon on this machine the contents of that repository's .kin/daemon.token" },
    EnvVarSpec { name: "KIN_REMOTE_AUTH_TOKEN", kind: Kind::Secret, default: "", sensitivity: Sensitivity::Secret, summary: "fallback bearer token for a native Kin remote, read after KIN_REMOTE_BEARER_TOKEN" },
...
pub const DOWNSTREAM: &[EnvVarSpec] = &[
    ...
    EnvVarSpec { name: "KIN_VFS_BIN", kind: Kind::Path, default: "", sensitivity: Sensitivity::Operational, summary: "pin the `kin-vfs` projection driver Kin probes and runs, instead of searching beside the `kin` binary, in `~/.kin/bin`, and on PATH. When set it is the only candidate, so a pin naming a file that is not there reports an absent driver rather than resolving to another one. Use it to run a driver built with a mount feature without reordering PATH" },
    ...
    EnvVarSpec { name: "KIN_VFS_WORKSPACE", kind: Kind::Path, default: "", sensitivity: Sensitivity::Correctness, summary: "kin-vfs projection root, exported by the shell hook into every process it starts; the shim answers every path under it from the graph and refuses it when it cannot, so a root nothing serves fails every path under it. `kin vfs status` reads it to report which root is bound here. Set by the hook rather than by hand" },
    EnvVarSpec { name: "KIN_VFS_SOCK", kind: Kind::Path, default: "", sensitivity: Sensitivity::Operational, summary: "kin-vfs daemon socket the shim connects to; unset means the bound root's own `.kin/vfs.sock`. `kin vfs status` probes it with a connect rather than a file test, because a socket file outlives the daemon that bound it" },
