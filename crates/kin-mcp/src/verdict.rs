...
/// of them: the observation is taken only when a question NAMES an unadmitted
/// module, so its presence is already the finding. There is no certified state
/// here on purpose. A response with no block did not establish that the question
/// stays inside the graph, it establishes that nobody looked, and an input that
