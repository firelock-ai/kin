...
pub fn handle_trace_data_flow<G: GraphStore>(
    ...
    while !frontier.is_empty() {
        ...
        for node in frontier.drain(..) {
            ...
            for candidate in callees.into_iter().chain(callers) {
                ...
                let mut step_value = trace_step_value(
                    step_index,
                    candidate.role,
                ...
                // trustworthy as its weakest hop, and a `name_only` hop means
                // the flow may not exist at all.
                step_value["resolution"] = serde_json::Value::from(candidate.resolution.as_str());
                chain.push(step_value);
                if next_depth < depth {
                    next_frontier.push(TraceFrontierNode::at(
                        step_index,
                        next_depth,
    ...
    // The budget is applied last so it accounts for every field above it,
    // including the coverage the envelope carries.
