...
/// tool's: a `certified`, `complete` answer about a package the graph never
/// admitted reads the same whichever surface produced it. The argument name is
/// asked for rather than assumed, because `get_context_pack` carries its
/// question as `question` where the rest carry `query`.
///
/// This is the OFFLINE route. The daemon serves several of these tools from its
/// own pipelines and returns before this function is reached, which is why
/// `kin-daemon`'s `mcp_tools_call` carries the same attachment at its own
/// chokepoint. Two call sites, one observation, because there are two routes.
///
...
fn attach_outside_graph<G: GraphStore>(
    ...
    let Some(question) = crate::outside_graph::question_argument(arguments) else {
        return;
    };
